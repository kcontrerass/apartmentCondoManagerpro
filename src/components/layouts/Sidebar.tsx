...
export function Sidebar({
        ...
        items: {
            ...
            },
            {
                ...
                items: [
                    ...
                    { icon: 'how_to_vote', label: t('polls'), href: '/dashboard/polls', roles: [Role.SUPER_ADMIN, Role.ADMIN, Role.BOARD_OF_DIRECTORS, Role.RESIDENT] },
                ],
            },
            {
                label: t('support'),
    ...
    const hasPermission = (href: string, role?: string) => {
        ...
        if (!key) return true; // Always show if no matching permission key (e.g., /dashboard)

        // Cascade restriction: if amenities is off, reservations must also be off
        if (key === 'reservations' && permissions['amenities'] === false) {
            return false;
        }
    ...
    return (
        <>
            ...
            <aside className={cn(
                "flex flex-col w-64 h-full bg-sidebar border-r border-card-border flex-shrink-0 fixed left-0 top-0 z-50 transition-transform duration-300",
                ...
                <div className="p-6 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <h1 className="text-foreground text-base font-bold leading-none tracking-tight">
                ...
                < nav className="flex-1 overflow-y-auto px-4 py-2 flex flex-col gap-1 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700" >
                    {
                        navSections.map((section) => {
                            const filteredItems = section.items.filter(item => {
                                const roleMatches = !item.roles || (user?.role && item.roles.includes(user.role as Role));
                                const permMatches = hasPermission(item.href, user?.role);
                                return roleMatches && permMatches;
                            });
                ...
                {/* User Profile */}
                <div className="p-4 border-t border-slate-200 dark:border-slate-800 mt-auto bg-slate-50/50 dark:bg-background-dark/50">
                    <div className="flex flex-col gap-3">
                        <Link href="/dashboard/profile" className="flex items-center gap-3 group min-w-0 p-2 rounded-xl hover:bg-white dark:hover:bg-slate-800 transition-all shadow-sm hover:shadow-md border border-transparent hover:border-slate-200 dark:hover:border-slate-700">
